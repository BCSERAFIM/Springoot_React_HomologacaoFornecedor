...
import com.bcserafim.homologacaoFornecedor.services.PaisService;
...
public class PaisResource {
	...
	public ResponseEntity<Pais> findById(@PathVariable Long id) {
	 ...
	 public ResponseEntity<Pais> insert(@RequestBody Pais obj) {
		 Pais novoPais = service.insert(obj);
		 URI uri = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(novoPais.getId()).toUri();
		 return ResponseEntity.created(uri).body(novoPais);
	 }
	 ...
	 @PutMapping(value = "/{id}")
	 public ResponseEntity<Pais> update(@PathVariable Long id, @RequestBody Pais obj ) {
